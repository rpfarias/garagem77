...
export const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ label, error, helpText, leftIcon, rightIcon, className, id, ...props }, ref) => {
    // useId() gera IDs estáveis entre server e client (evita hydration mismatch)
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="relative">
          ...
          <input
            ...
            id={inputId}
            className={clsx(
              'w-full px-3.5 py-2.5 rounded-lg text-sm bg-white dark:bg-slate-900',
              ...
              leftIcon && 'pl-10',
              rightIcon && 'pr-10',
              error && 'border-danger-300 dark:border-danger-700 focus:border-danger-500 focus:ring-danger-500/10',
              className
            )}
            {...props}
          />
        ...
        {helpText && !error && (
          <span className="text-xs text-slate-500 dark:text-slate-400">{helpText}</span>
        )}
      </div>
    );
  }
